...
grid_2022_mammals$realms = r_grid_soc_eco$REALM
load('grid_2022_amphibians.Rdata')

grid_2022_amphibians = grid_2022
grid_2022_amphibians$realms = r_grid_soc_eco$REALM
...
theme_harith=theme(legend.key.height = unit(0.2,'cm'),
...
grob7 <- circle_plot(grid_2022_amphibians[grid_2022_amphibians$land == 1,]$quant9_dif_ri)
...
p1_points_a = ggplot(data = grid_2022_amphibians,aes(x = log10(WEGE_2012),y = log10(WEGE),col = realms))+
  ...
  annotate(geom="text", x=0, y=-6.3, label="WEGE(log10)",size = 3)+
  ylab('2023')+
  xlab('2012') +labs(subtitle = 'D')

p2_points_a = ggplot(data = grid_2022_amphibians,aes(x = Richness_2012,y = Richness,col = realms))+
  geom_point(size = 0.5,alpha = 0.25)+
  theme_minimal()+
  theme(legend.position = 'none',axis.title = element_text(size = 7),
        axis.text = element_text(size = 5))+  geom_abline(intercept = 0, slope = 1,col = 'black')+
  theme(plot.subtitle = element_text(family = 'bold'))+
  scale_colour_manual(values = pal)+
  annotate(geom="text", x=140, y=10, label="Richness",size = 3)+
  ylab('2023')+
...
p_rich9_a = ggplot()+
  ...
  theme_void()+
  theme_harith+
  annotate(geom="text", x=-14702330, y=-8020048, label="Richness(10th)",size = 3)+
  labs(subtitle = 'A')

p_rich9_a <- p_rich9_a + annotation_custom(grob = grob7, xmin = 13009400, xmax = 17899400, ymin = -Inf, ymax = -4000000)
...
p1_a = ggplot()+
  ...
  annotate(geom="text", x=-13702330, y=-8020048, label="WEGE(10th quantile)",size = 3)+
  labs(subtitle = 'C')
...
grob7 <- circle_plot(grid_2022_mammals[grid_2022_mammals$land == 1,]$quant9_dif_ri)
...
p_rich9_m = ggplot()+
  geom_sf(data = worldmap,bg = 'grey80',col = 'grey80')+
  geom_sf(data = grid_2022_mammals[grid_2022_mammals$land == 1 & grid_2022_mammals$quant9_dif_ri>0,],aes(col = as.factor(quant9_dif_ri),fill=as.factor(quant9_dif_ri)),linewidth = 0)+
  scale_fill_manual('',values = pal,labels = c('2012','2022','Both'),na.translate = F)+
  scale_colour_manual('',values = pal,labels = c('2012','2022','Both'),na.translate = F)+
  theme_void()+
  theme_harith+
...
p_rich9_m <- p_rich9_m + annotation_custom(grob = grob7, xmin = 13009400, xmax = 17899400, ymin = -Inf, ymax = -4000000)
...
p1_points_m = ggplot(data = grid_2022_mammals,aes(x = log10(WEGE_2012),y = log10(WEGE),col = realms))+
  ...
  scale_colour_manual(values = pal)+
  annotate(geom="text", x=-0.75, y=-6.3, label="WEGE(log10)",size = 3)+
  ylab('2023')+
  xlab('2012')+
  labs(subtitle = 'H')
...
p2_points_m = ggplot(data = grid_2022_mammals,aes(x = Richness_2012,y = Richness,col = realms))+
  geom_point(size = 0.5,alpha = 0.25)+
  theme_minimal()+
  theme(legend.position = 'none',axis.title = element_text(size = 7),
        axis.text = element_text(size = 5))+  geom_abline(intercept = 0, slope = 1,col = 'black')+
  theme(plot.subtitle = element_text(family = 'bold'))+
  scale_colour_manual(values = pal)+
  annotate(geom="text", x=210, y=10, label="Richness",size = 3)+
    ylab('2023')+
  xlab('2012') +labs(subtitle = 'F')

grid_all = arrangeGrob(p_rich9_a,p2_points_a,p1_a,p1_points_a,p_rich9_m,p2_points_m,p1_m,p1_points_m, ncol=2)

ggsave(plot = grid_all,filename = 'grid_all_complete_jun_27.png',width = 9,height = 10,dpi = 2000)
